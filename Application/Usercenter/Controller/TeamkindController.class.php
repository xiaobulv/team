...
class TeamkindController extends BaseController
{
	...
	public function lists($model = null, $p = 0){
		$model = M('Model')->where(array('name' => 'team_kind'))->find();
	...
	public function add(){
		//获取分类信息
		$id = is_login();
		$kind = M('team_kind')->where('uid = '.$id)->select();
		$this->assign('kind',$kind);
        //获取模型信息
        $model = M('Model')->where(array('name' => 'team_kind'))->find();
        $model || $this->error('模型不存在！');
		//file_put_contents("log.php",var_export($_POST,TRUE));
        if(IS_POST){
			$_POST['uid'] =is_login();
			$_POST['kindtime'] =chr(rand(97,122)).time();

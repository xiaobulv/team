...
class TeamController extends BaseController
{
    ...
    {
        //调用API获取基本信息
        $user = query_user(array('avatar256'), $uid);
        //显示页面
	...
	public function team(){
			$uid = $_GET['uid'];
			$kind = M('team_kind')->where('uid = '.$uid)->order('sort desc')->select();//团队分类
			$member = M('team')->where('uid = '.$uid)->select();//团队成员
			foreach($member as $value){
				$array[$value['id']] = explode(",",$value['type']);
			}
			$team_style = D('Member')->where(array('uid'=>$uid))->field('team_style')->find();
			if(empty($team_style['team_style'])){
				$team_style = 'team_1';
			}else{
				$team_style = $team_style['team_style'];
			}
			$info = D('Member')->where('uid ='.$uid)->field('weibo,my_url')->find();
			if(empty($info['weibo'])){
				$info['weibo'] ="http://weibo.com/chuangxin361";
			}
            ...
            $this->display($team_style);
        }
	public function style(){//切换风格
        	if(IS_POST){
				$_POST['uid'] = is_login();
				$Model = D('Member');
				...
				$info = D('Member')->where('uid ='.is_login())->field('weibo,my_url')->find();
				$this->assign('info', $info);
			}
           	 $this->display();
     }
	public function lists($model = null, $p = 0){
		$model = M('Model')->where(array('name' => 'team'))->find();
		...
		foreach ( $data as $k=>$vo ) {
			if (! empty ( $vo ['head'] ))
				$data[$k] ['head'] = '<img src="' .__ROOT__. get_cover ( $vo ['head'] , 'path' ) . '" width="50" height="50" />';
		}
		foreach ( $data as $z=>$vo ) {
			$id = explode(',',$vo['type']);
			$data[$z]['type'] ="";
			foreach($id as $k ){
				$data[$z]['type'] .= get_team_type($k)." ";
			}
		}

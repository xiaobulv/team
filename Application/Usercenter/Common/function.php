 ...
 * 获取用户分类
 * @param intger $id 
 * @author guofeng <guofenglov@gmail.com>
 */
function get_team_type($id){
	
	$uid = is_login();
	$kind = M('team_kind')->where('uid = '.$uid)->getField('id,kind');
	if($kind){
		return $kind[$id];
	}else{
		return;
...
function get_team_alltype(){
	$uid = is_login();
	$kind = M('team_kind')->where('uid = '.$uid)->select();
	if($kind){
		foreach($kind as $vo){
			$res[$vo['id']] = $vo['kind'];
		}
...
function getKind($id){
	$kind = M('team_kind')->where('id ='.$id)->field('kindtime')->find();
	if($kind)
...
function getColor($id){
	$count = M('team_kind')->where('id <='.$id)->field('kindtime')->count();
	if($count)
		return $count+1;
	else
		return;
